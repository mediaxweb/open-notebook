...
def note_panel(note_id, notebook_id=None):
    ...
    t_preview, t_edit = st.tabs(["Xem trước", "Chỉnh sửa"])
    with t_preview:
        st.subheader(note.title)
        st.markdown(convert_source_references(note.content))
    with t_edit:
        note.title = st.text_input("Tiêu đề", value=note.title)
        note.content = st_monaco(
            value=note.content, height="300px", language="markdown"
        )
        b1, b2 = st.columns(2)
        if b1.button("Cập nhật", key=f"pn_edit_note_{note.id or 'new'}"):
            logger.debug("Editing note")
            note.save()
            if not note.id and notebook_id:
                note.add_to_notebook(notebook_id)
            st.rerun()
    if b2.button("Xóa", type="primary", key=f"delete_note_{note.id or 'new'}"):
